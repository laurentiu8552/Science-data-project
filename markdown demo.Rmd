...
# Add cross-validation and k parameter tuning
# First, let's set up a function to evaluate different k values
evaluate_k_values <- function(data, k_values) {
  results <- list()
  
  for (k in k_values) {
    # Create evaluation scheme with k-fold cross-validation
    scheme <- evaluationScheme(data, method = "cross-validation", k = 5, given = -1)
    
    # Evaluate with current k parameter
    eval_results <- evaluate(scheme, method = "IBCF", parameter = list(k = k), n = c(1, 3, 5, 10))
    
    # Store results
    results[[as.character(k)]] <- eval_results
  }
...
# Run evaluation
cv_results <- evaluate_k_values(filtered_ratings, k_values)

# Extract and compare results
comparison <- lapply(names(cv_results), function(k) {
  # Get average metrics for each k value
  res <- avg(cv_results[[k]])
  data.frame(
    k = k,
    TP_Rate = res["TPR", "5"],  # True Positive Rate at n=5
    Precision = res["precision", "5"],  # Precision at n=5
    Recall = res["recall", "5"]  # Recall at n=5
  )
})
...
# Find the best k value based on precision
best_k <- as.numeric(comparison_df$k[which.max(comparison_df$Precision)])
print(paste("Best k value based on precision at n=5:", best_k))

# Visualize results if desired
if (require(ggplot2)) {
  ggplot(comparison_df, aes(x = k, y = Precision, group = 1)) +
    geom_line() +
    geom_point() +
    labs(title = "Precision at n=5 for different k values",
         x = "Number of neighbors (k)",
         y = "Precision")
}

# Train final model with the best k
final_recommender <- Recommender(filtered_ratings, method = "IBCF", 
                                parameter = list(k = best_k))

# Make predictions
n_recommended <- 5
final_predictions <- predict(final_recommender, newdata = test_data, n = n_recommended)
final_recommendation_list <- as(final_predictions, "list")
names(final_recommendation_list) <- rownames(test_data)

# Create final output
final_recommendation_df <- data.frame(
  customer_id = names(final_recommendation_list),
  recommended_items = sapply(final_recommendation_list, paste, collapse = ", "),
  k_value_used = best_k
)

# Join with RFM data
final_recommendation_df$customer_id <- as.character(final_recommendation_df$customer_id)
final_output <- final_recommendation_df %>%
