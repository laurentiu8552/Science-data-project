...
```{r ## Association Rules mining}
# read data
data1 <- read_excel("jysk_case_competition_final.xlsx")
...
summary(data1)
# date             order_id          customer_id          product_id      order_value_ex_vat_ex_freight product_title     
# Length:424965      Length:424965      Min.   :1.018e+04   Min.   :1011480   Min.   : -16386.4             Length:424965     
# Class :character   Class :character   1st Qu.:1.018e+08   1st Qu.:3600116   1st Qu.:     75.0             Class :character  
# Mode  :character   Mode  :character   Median :1.019e+08   Median :3674174   Median :    214.3             Mode  :character  
# Mean   :3.960e+08   Mean   :3924440   Mean   :    877.3                               
# 3rd Qu.:1.018e+09   3rd Qu.:4541308   3rd Qu.:    675.4                               
# Max.   :1.018e+09   Max.   :7399680   Max.   :1720890.6                               
# NA's   :19111       NA's   :365                                                       
# 
# customer_zip_code  product_group_level_1 product_category_level_2
# Length:424965      Length:424965         Length:424965           
...
data1[] <- lapply(data1, function(x) if (is.character(x)) as.factor(x) else x)

str(data1)
glimpse(data1)

trandata1 <- plyr::ddply(data1, c("date","customer_id"),
                         function(df1)paste(df1$product_title, collapse = ","))
head(trandata1)
View(trandata1)

trandata1$customer_id <- NULL
trandata1$date <- NULL

colnames(trandata1) <- c("items")
head(trandata1)

write.csv(trandata1, "mbt.csv", quote = FALSE, row.names = FALSE)

tr <- read.transactions('mbt.csv', format = 'basket', sep = ',')
...
inspect(shorter.association.rules[1:3])
