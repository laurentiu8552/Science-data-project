...
itemFrequencyPlot(tr,topN=25,type="absolute",col=brewer.pal(8,'Pastel2'), 
...
-   At 0.8 confidence 3 rules were found: {MA 60x186x12} =\> {5cm PLUS F15 chair} {5cm PLUS F15 chair} =\> {MA 60x186x12} {Back cushion DUNHAMMER 35x75 beige} =\> {BTH DUNHAMMER 220x240 beige}

The first 3 in particular have a lift of 776 indicating a strong relationship of the items bought together
...
-   Due to the relaxed support and confidence now a list of 17 of items bought together.
...
-   The plot provides the situation of the 17 rules found previously and mapping them on the scale of confidence and support, the best ones are the ones situated at the highest confidence and highest support

```{r plot for 17 rules and their respective confidence and support}
# interactive plot
...
-   The plot provides a representation of the order 2 and 3 rules. THe order 3 ones are clearly low in support and lower in confidence compared the order 2 ones.
...
-   The plot shows a clear interrelationship between the top 5 order 3 rules. Their confidence is above 0.6, but a high lift around 700 - it provides insights into plate and bowl products
-   A circular relationship with a high lift and confidence is seen in the products MA 60x186x12 and 5cm PLUS F15 chair.
-   Additionally a relationship which point to the same bought product {Basket INFINITY 4} happens upon the purchase of {5L plastic white} and {5L plastic grey}. The rules are high in lift around 1000 and with a confidence of 1.
...
```{r}
# Improved Recommender System with Multiple Models and Advanced Features
...
# Step 1: Data Preparation with Time-based Weighting
# Assuming data_rcs has customer_id, product_title, purchase_date, and purchase_amount
data_rcs <- data

# Create weighted interaction dataset considering recency and purchase value
interactions <- data_rcs %>%
  dplyr::select(customer_id, product_title, purchase_date, purchase_amount) %>%
  mutate(
    days_since = as.numeric(Sys.Date() - purchase_date),
    recency_weight = exp(-0.01 * days_since),  # Exponential decay for recency
    purchase_strength = scale(purchase_amount)[,1],  # Standardized purchase amount
    interaction_weight = 1 + (recency_weight * 0.5) + (purchase_strength * 0.5)  # Combined weight
  )

# Create binary and weighted interaction datasets
...
weighted_interactions <- interactions %>%
  ...
  summarize(
    ...
    recent_purchase = max(purchase_date),
    weighted_score = sum(interaction_weight),
    .groups = "drop"
  )

# Step 2: Sample Customers (for development/testing purposes)
# For a production system, you might use all customers
sample_size <- min(5000, length(unique(interactions$customer_id)))
sample_customers <- sample(unique(interactions$customer_id), sample_size)
sample_data <- binary_interactions %>%
  filter(customer_id %in% sample_customers)

# Step 3: Product Selection with Popularity Dampening
# This helps reduce bias toward universally popular items
product_counts <- sample_data %>%
  dplyr::count(product_title, sort = TRUE)

# Apply logarithmic dampening to reduce popularity bias
product_counts <- product_counts %>%
  mutate(
    log_count = log1p(n),
    dampened_score = log_count / (1 + log1p(n) * 0.1)  # Dampening factor
  ) %>%
  arrange(desc(dampened_score))

# Select top products after dampening (increased from 100 to 250)
...
# Step 4: Create User-Item Matrices (both binary and weighted)
# Binary matrix
binary_matrix <- sample_data %>%
  tidyr::pivot_wider(
...
# Store customer IDs
customer_ids <- binary_matrix$customer_id
binary_matrix <- binary_matrix %>% dplyr::select(-customer_id)
binary_mat <- as.matrix(binary_matrix)
rownames(binary_mat) <- customer_ids

# Weighted matrix (for advanced models)
weighted_data <- weighted_interactions %>%
  filter(customer_id %in% sample_customers,
         product_title %in% top_products)

weighted_matrix <- weighted_data %>%
  tidyr::pivot_wider(
    id_cols = customer_id,
    names_from = product_title,
    values_from = weighted_score,
    values_fill = list(weighted_score = 0)
  )

weighted_customer_ids <- weighted_matrix$customer_id
weighted_matrix <- weighted_matrix %>% dplyr::select(-customer_id)
weighted_mat <- as.matrix(weighted_matrix)
rownames(weighted_mat) <- weighted_customer_ids

# Step: 5 Convert to recommenderlab format
binary_ratings <- as(binary_mat, "binaryRatingMatrix")
weighted_ratings <- as(weighted_mat, "realRatingMatrix")

# Step 6: Filter sparse data with improved thresholds
min_items_per_user <- 3
min_users_per_item <- 5

# Apply filtering
filtered_binary <- binary_ratings[
  rowCounts(binary_ratings) >= min_items_per_user,
  colCounts(binary_ratings) >= min_users_per_item
]

filtered_weighted <- weighted_ratings[
  rowCounts(weighted_ratings) >= min_items_per_user, 
  colCounts(weighted_ratings) >= min_users_per_item
]

# Step 7: Create evaluation scheme with cross-validation
n_folds <- 5

# For binary data
scheme_binary <- evaluationScheme(
  filtered_binary,
  method = "cross-validation",
  k = n_folds,
  given = 3,  # Number of items to use as training data for each test user
  goodRating = 1
)

# For weighted data
scheme_weighted <- evaluationScheme(
  filtered_weighted,
  method = "cross-validation",
  k = n_folds,
  given = 3
...
# Step 8: Define multiple recommendation algorithms to compare
algorithms <- list(
  IBCF_cos = list(name = "IBCF", param = list(k = 30, method = "cosine")),
  IBCF_cor = list(name = "IBCF", param = list(k = 30, method = "pearson")),
  UBCF = list(name = "UBCF", param = list(method = "cosine", nn = 25)),
  SVD = list(name = "SVD", param = list(k = 10)),
  Pop = list(name = "POPULAR", param = NULL),
  # Adding a hybrid approach that combines IBCF and POPULAR
  Hybrid = list(name = "HYBRID", param = list(
    recommenders = c("IBCF", "POPULAR"),
    weights = c(0.7, 0.3)
  ))
)

# Step 9: Evaluate multiple models (using parallel processing for speed)
n_cores <- detectCores() - 1
cl <- makeCluster(n_cores)

# Function to evaluate a single model
evaluate_model <- function(scheme, algorithm, ns) {
  recommenderlab::evaluate(scheme, 
                          method = algorithm$name, 
                          param = algorithm$param, 
                          n = ns)
}

# For binary data - evaluate all models
ns <- c(1, 5, 10, 15)
binary_results <- lapply(algorithms, function(algo) {
  evaluate_model(scheme_binary, algo, ns)
})
names(binary_results) <- names(algorithms)

# For weighted data - evaluate models that support real-valued ratings
weighted_algorithms <- algorithms[c("IBCF_cos", "IBCF_cor", "UBCF", "SVD")]
weighted_results <- lapply(weighted_algorithms, function(algo) {
  evaluate_model(scheme_weighted, algo, ns)
})
names(weighted_results) <- names(weighted_algorithms)

stopCluster(cl)

# Step 10: Compare and visualize results
binary_avg_results <- sapply(binary_results, function(x) {
  getConfusionMatrix(x)$prec
})

# Plot comparison of precision for different algorithms
results_df <- data.frame(
  Algorithm = rep(names(binary_results), each = length(ns)),
  N = rep(ns, times = length(binary_results)),
  Precision = as.vector(binary_avg_results)
)

precision_plot <- ggplot(results_df, aes(x = N, y = Precision, color = Algorithm, group = Algorithm)) +
  geom_line() +
  geom_point() +
  theme_minimal() +
  labs(title = "Precision Comparison of Recommendation Algorithms",
       x = "Number of Recommendations",
       y = "Precision")

# Step 11: Select the best performing model based on evaluation
# In this case, we'll choose the model with the highest precision at n=5
best_model_idx <- which.max(binary_avg_results[ns == 5, ])
best_model_name <- names(binary_results)[best_model_idx]
best_model_params <- algorithms[[best_model_name]]$param
...
recommender_model <- Recommender(
  filtered_binary, 
  method = algorithms[[best_model_name]]$name,
  parameter = best_model_params
)

# Step 13: Generate recommendations for all users
n_recommended <- 10
all_predictions <- predict(recommender_model, filtered_binary, n = n_recommended, type = "topNList")
recommendation_list <- as(all_predictions, "list")

# Step 14: Create recommendations dataframe
...
for (i in 1:n_recommended) {
  col_name <- paste0("recommendation_", i)
  recommendation_df[[col_name]] <- sapply(recommendation_list, function(x) {
    if (length(x) >= i) return(x[i]) else return(NA)
  })
}

# Also add concatenated recommendations
recommendation_df$all_recommendations <- sapply(recommendation_list, paste, collapse = ", ")

# Step 15: Integrate with RFM segmentation
# Assuming segment dataframe exists with customer RFM data
rfm_sample <- segment %>%
  filter(customer_id %in% recommendation_df$customer_id)
...
# Step 16: Add explanation for recommendations
if (best_model_name %in% c("IBCF_cos", "IBCF_cor")) {
  # Extract item similarity matrix for recommendation explanation
  model_details <- getModel(recommender_model)
  sim_matrix <- model_details$sim
  
  # Function to get similar items that led to recommendation
  get_explanation <- function(user_id, item_id, top_n = 3) {
    # Get user's purchased items
    user_items <- names(which(as.vector(filtered_binary[user_id, ]) == 1))
    
    # Get similarity scores between recommended item and user's items
    if (length(user_items) == 0) return("Based on popularity")
    
    item_similarities <- sim_matrix[item_id, user_items, drop = FALSE]
    if (length(item_similarities) == 0) return("Based on popularity")
    
    # Sort and get top similar items
    sorted_items <- sort(item_similarities, decreasing = TRUE)
    top_items <- names(sorted_items)[1:min(top_n, length(sorted_items))]
    
    paste("Because you purchased:", paste(top_items, collapse = ", "))
  }
  
  # Add explanation for the first recommendation
  recommendation_df$explanation <- mapply(
    get_explanation,
    recommendation_df$customer_id,
    recommendation_df$recommendation_1
  )
}

# Step 17: Segment-specific recommendations
# Optional: create separate models for different RFM segments
# Here we simply analyze how recommendations differ by segment

segment_analysis <- final_output %>%
...
# write.csv(final_output, "personalized_recommendations.csv", row.names = FALSE)
# save(recommender_model, file = "recommender_model.RData")

# Return the final recommendations and model comparison
list(
...
# filter RFM to match 
rfm_sample <- segment %>%
  filter(customer_id %in% names(recommendation_list))  # safer than rownames(mat)
