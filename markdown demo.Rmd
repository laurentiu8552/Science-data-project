...
order_level_data <- data_clean_rfm %>%
  dplyr::select(date, order_id, customer_id, revenue) %>%  # select only relevant columns
  dplyr::group_by(order_id, customer_id, date) %>%         # group by order, customer, and date
  dplyr::summarise(total_revenue = sum(revenue, na.rm = TRUE)) %>%  # sum revenue for each order
  dplyr::ungroup()
#This makes a new aggregated data frame on order level instead of product level
