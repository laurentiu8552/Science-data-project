...
```{r Running the CLV models}
#### 1 model
library(BTYD)

 # Select subset
    clv_subset <- mutate(clv_data[,c(1,2,3)], date =as.Date(date)) # gets only needed columns 
...
hist(rowCounts(binary_ratings), 
     main = "Number of Products Purchased per Customer",
     xlab = "Products", col = "lightblue")

# most purchase one

# how many customers have purchased each product
hist(colCounts(binary_ratings),
     main = "Number of Customers per Product",
     xlab = "Customers", col = "salmon")

# top N most purchased products
top_products <- data.frame(
  product_title = colnames(binary_ratings),
  purchase_count = colCounts(binary_ratings)
) %>%
  arrange(desc(purchase_count)) %>%
  slice(1:10)

print(top_products)
# same as association rules mining

# plot top products
library(ggplot2)

ggplot(top_products, aes(x = reorder(product_title, purchase_count), y = purchase_count)) +
...
# Keep customers with more than 5 purchases and products purchased by at least 20 customers
filtered_ratings <- binary_ratings[rowCounts(binary_ratings) > 5, colCounts(binary_ratings) > 20]
...
# Step 7: Train the recommender model (User-Based Collaborative Filtering)
recommender_model <- Recommender(binary_ratings, method = "UBCF")

# Step 8: Generate recommendations for first 5 customers
recommendations <- predict(recommender_model, binary_ratings[1:5], n = 5)
